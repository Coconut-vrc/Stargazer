...
- **2つのモード**:
  - **exclude**: マッチング時に NG ペアを自動排除（**全ロジック共通で絶対排除**）
  - **warn**: マッチングは通常通り実行し、NG ペアにフラグ付与
- **NG排除の統一方針**: excludeモードにおいてNGは全ロジック（M001/M002/M003）で絶対排除。NG排除が不可能な場合、マッチングは不成立とし、UIで警告モーダルを表示する。サイレントにNGを無視することはない
...
1. **処理順序バイアス**
   ...
   - 後半のユーザーは選択肢が少なくなりがち
   - **影響度**: 中〜高。ラウンド数が多ければシャッフルにより偏りは平均化されるが、**ラウンド数が少ない場合（2ラウンド等）は偏りが残りやすい**
...
- **NG絶対排除**: 全オフセットが NG で無効な場合、マッチング不成立として `ngConflict` フラグを返しUIで警告（サイレントフォールバックなし）

#### ⚠️ 課題・改善余地

1. ~~**ユーザー順序依存**~~ → **修正済み**
   - ~~`winners` 配列の順序がそのままスロット配置に使われていた（シャッフルなし）~~
   - `shuffledWinners` を導入し、M001/M003 と同様にランダムシャッフルを適用済み
   - **影響度**: 解消済み
...
- **NG絶対排除**: 全オフセットが NG で無効な場合、マッチング不成立として `ngConflict` フラグを返し、UIで警告モーダルを表示（NGを無視したフォールバックは行わない）
...
3. **NG排除がオフセット単位**
   ...
   - 有効なオフセットが0件の場合、マッチングは不成立として警告モーダルを表示
   - **対処法**: 抽選のやり直し、NGの原因となるキャストの欠席設定、NGユーザー設定の見直し
   - **影響度**: 中。NG ユーザーが多い場合は対処が必要
...
> [!NOTE]
> **M002 のユーザーシャッフル欠如** → **修正済み**
> 
> `rotation-sheets.ts` に `shuffledWinners` を導入し、M001/M003 と同様のシャッフルを適用。

> [!NOTE]
> **M002/M003 の NG フォールバック** → **修正済み**
> 
> 旧仕様: 有効オフセットが0件の場合、M002は `offset=0` にサイレントフォールバック、M003はNGを無視してスコア最大を選択（**NGの意味がロジック途中で変質する危険な仕様**）
> 
> 新仕様: M002/M003 とも有効オフセットが0件の場合、マッチング不成立として `ngConflict: true` を返し、
> UI で警告モーダルを表示。ユーザーに「抽選やり直し」「キャスト欠席設定」「NG設定見直し」を促す。
> NGは全ロジック（M001/M002/M003）で絶対排除として統一。
