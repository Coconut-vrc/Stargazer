...
export const ImportPage: React.FC<ImportPageProps> = ({ onImportUserRows }) => {
  ...
  const handleCustomImport = () => {
    ...
    const fixedIndices = new Set(
      [customMapping.name, customMapping.x_id, customMapping.cast1, customMapping.cast2, customMapping.cast3, customMapping.cast4, customMapping.cast5].filter(
        (i) => i >= 0
      )
    );
    ...
    const finalMapping =
      castInputType === 'multiple'
        ? { ...mappingWithExtra, cast2: -1, cast3: -1, cast4: -1, cast5: -1 }
        : mappingWithExtra;
